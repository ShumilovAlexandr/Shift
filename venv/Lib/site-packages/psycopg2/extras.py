...
class RealDictCursor(DictCursorBase):
    ...
    to access auth rows both as a dictionary and a list, then use
...
def register_hstore(conn_or_curs, globally=False, unicode=False,
    ...
    :param unicode: if `!True`, keys and values returned from the auth
    ...
    auth and look for the OID of the |hstore| type (which may be different
    ...
    *oid* parameter, which can be found using a query such as :sql:`SELECT
    ...
    Note that, when passing a dictionary from Python to the auth, both
    strings and unicode keys and values are supported. Dictionaries returned
    from the auth have keys/values according to the *unicode* parameter.

    The |hstore| contrib module must be already installed in the auth
    ...
    if oid is None:
        ...
        if oid is None or not oid[0]:
            raise psycopg2.ProgrammingError(
                "hstore type not found in the auth. "
...
class CompositeCaster:
    ...
    querying the auth at registration time is not desirable (such as when
